...
sub parse_options {
    ...
    # Declare lexical variables that will be added to the configuration
    ...
    my ($opt, $usage)= describe_options(
        "Usage: compadre_kickoff.pl %o\n",
            ...
            [ 'help|h', "COMPADRE help message", { shortcircuit => 1 } ],      
            ...
            [ 'output_dir=s', "Specify path to the output directory for all results" ],   
            ...
            [ 'reference_pop=s', "Reference populations"],
    ...
    if (defined $opt->sexes) {
        ...
        my $sex_hash = $opt->sexes;
        foreach my $raw_key (keys %$sex_hash) {
            my $value = $sex_hash->{$raw_key};
            my $key = uc($raw_key);
            if (grep { $_ eq $key } @possible_keys) {
                $sexes{$key} = $value;
            } else {
    ...
    if (defined $opt->mito_data) {
        ...
        my $mito_hash = $opt->mito_data;
        foreach my $raw_key (keys %$mito_hash) {
            my $value = $mito_hash->{$raw_key}; 
            my $key = uc($raw_key);
            if (grep { $_ eq $key } @possible_keys) {
    ...
    if (defined $opt->y_data) {
        my @possible_keys = qw(FILE FID1 IID1 FID2 IID2 MATCH MATCH_VAL);
        my $ydata_hash = $opt->y_data;
        foreach my $raw_key (keys %$ydata_hash) {
            my $value = $ydata_hash->{$raw_key};
            my $key = uc($raw_key);
            if (grep { $_ eq $key } @possible_keys) {
                $y_matches{$key} = $value;
            } else {
    ...
    if (defined $opt->affections) {
        ...
        my $affections_hash = $opt->affections;
        foreach my $raw_key (keys %$affections_hash) {
            my $value = $affections_hash->{$raw_key};
            my $key = uc($raw_key);
            if (grep { $_ eq $key } @possible_keys) {
