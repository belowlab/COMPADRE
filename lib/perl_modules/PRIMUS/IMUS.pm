...
sub run_IMUS {
    my ($cli_config, $state) = @_;

    my $config;
    if (ref($cli_config) eq 'HASH' && exists $cli_config->{imus}) {
        $config = {
            threshold            => $cli_config->{imus}->{relatedness_threshold},
            min_likelihood       => $cli_config->{imus}->{initial_likelihood_cutoff},
            exclude_value        => $cli_config->{imus}->{missing_data_value},
            lowest_max_network_size => 60,
            output_dir            => $cli_config->{global}->{output_dir},
            ibd_file_ref          => $cli_config->{imus}->{ibd_estimates},
            relatedness_file      => $cli_config->{imus}->{ibd_estimates}->{FILE},
            relatedness_file_name => do {
                my $f = $cli_config->{imus}->{ibd_estimates}->{FILE};
                if (defined $f && $f =~ /\/([^\/]+)$/) {
                    $1;
                } else {
                    $f // "relatedness_output.txt";
                }
            },
            samples_file          => $cli_config->{preprimus}->{samples} // $cli_config->{imus}->{samples} // "",
            trait_order          => $cli_config->{imus}->{trait_order} // [],
            trait_files          => $cli_config->{imus}->{traits} // {},
            relatedness_column   => ($cli_config->{imus}->{ibd_estimates}->{PI_HAT} // 10) - 1,
            id1_column           => ($cli_config->{imus}->{ibd_estimates}->{IID1} // 2) - 1,
            id2_column           => ($cli_config->{imus}->{ibd_estimates}->{IID2} // 4) - 1,
            fid1_column          => ($cli_config->{imus}->{ibd_estimates}->{FID1} // 1) - 1,
            fid2_column          => ($cli_config->{imus}->{ibd_estimates}->{FID2} // 3) - 1,
            do_IMUS              => $cli_config->{imus}->{get_max_unrelated_set} // 1,
            do_PR                => $cli_config->{imus}->{reconstruct_pedigrees} // 1,
            verbose              => $cli_config->{global}->{verbose} // 1,
            lib_dir              => $cli_config->{global}->{lib_dir},
        };
        
        # Ensure 'size' is in trait_files / trait_order if empty
        if ( !exists $config->{trait_files}{'size'} ) {
            $config->{trait_files}{'size'} = 'size';
            push( @{ $config->{trait_order} }, 'size' );
        }
        
        $do_IMUS = $config->{do_IMUS};
        $do_PR = $config->{do_PR};
        ...
        $config = PRIMUS::IMUS::Config->new();
        $state = PRIMUS::IMUS::State->new();
    ...
    # Verify all samples are assigned to networks (handles orphaned samples from missing pairs)
    # verify_all_samples_in_networks($config, $state);
...
# @purpose Establish canonical ordering of ID pairs for consistent hash lookups
# @param $id1 - First identifier (string ID or integer ID reference)
# @param $id2 - Second identifier (string ID or integer ID reference)
# @return (list) - Two-element list ($smaller, $larger) ordered by numeric/lexicographic comparison
# @notes Ensures that pair (A, B) and pair (B, A) always map to the same hash key location
#        Critical for memory optimization: allows single storage in $id_id_all_info{$smaller}{$larger}
#        and $id_id_scores{$smaller}{$larger} regardless of input order
...
# @purpose Create unique numeric mappings for IID/FID pairs and maintain reverse lookups
# @description Maps a pair of IID and FID strings to a unique numeric ID. Stores reverse mappings (numeric_id -> original_string) in both iid_map and fid_map within the state object, enabling efficient reconstruction of original ID strings from their numeric mappings during output generation. Tracks processed IIDs to avoid duplicate mappings.
# @param $iid_str (scalar) - Individual ID string to map (e.g., "id1", "sample_001")
# @param $fid_str (scalar) - Family ID string associated with this individual (e.g., "fam_A")
# @param $state (hashref) - State object containing:
#                           - iid_map: reverse mapping numeric_id -> original_iid_str
#                           - fid_map: reverse mapping numeric_id -> original_fid_str
#                           - ids_loaded: tracking hash to prevent duplicate mappings
# @return (void) - Modifies state object in place with new reverse mappings
# @side_effects Updates $state->{iid_map}, $state->{fid_map}, $state->{ids_loaded}, and increments internal counter on first encounter of an IID. Prints debug message with mapping.
# @usage generate_id_mapping($iid_str, $fid_str, $state); 
# Later, retrieve original strings: my $orig_iid = $state->{iid_map}->{$numeric_id};

sub generate_id_mapping {
    my ($iid_str, $fid_str, $state) = @_;
    state $counter = 1;

    if (!exists $state->{ids_loaded}->{$iid_str}) {
        $state->{iid_map}->{$counter} = $iid_str;
        $state->{fid_map}->{$counter} = $fid_str;
        $state->{ids_loaded}->{$iid_str} = $counter;
        $counter++;
    }
    return $state->{ids_loaded}->{$iid_str};
}


# @purpose Pack numeric IBD fields from a genome file line into binary format for memory optimization
# @description Converts seven numeric IBD relationship fields (Z0, Z1, Z2, PHE, DST, PPC, RATIO) from a space-separated genome file line into a compact binary representation. Handles edge cases like NA values in RATIO field. This packing reduces storage from ~159 bytes per line to ~32 bytes per line for the numeric data, enabling more efficient memory usage for large IBD datasets.
# @param $plink_file_indices (hashref) - Hash mapping field names to 0-based column indices in genome file (e.g., {Z0 => 6, Z1 => 7, ...}). Typically from config->{genome_file_columns}
# @param $line_array (arrayref) - Array of split fields from a single genome file line
# @return (string) - Binary packed data containing the seven IBD fields: (f f f s f f f) Layout: Z0(float) Z1(float) Z2(float) PHE(short) DST(float) PPC(float) RATIO(float)
# @modifies $line_array - Converts "NA" RATIO values to -1.0 in the array for packing
# @side_effects None beyond parameter modification
# @notes The pack template "f f f s f f f" uses 32-bit floats and one 16-bit short integer for ~32 bytes total The -1.0 sentinel value for missing RATIO enables numeric packing without special handling in unpack

sub pack_line {
    my ($plink_file_indices, $line_array) = @_;

    # Fallback to default indices if undefined or empty
    if (!defined $plink_file_indices || !ref($plink_file_indices) || keys %$plink_file_indices == 0) {
        $plink_file_indices = {
            FID1   => 0,
            IID1   => 1,
            FID2   => 2,
    ...
    # Handle the case where the RATIO column is "NA" by converting it to the nonsensical value of -1.0. This allows us to pack the data.
    my $ratio_idx = $plink_file_indices->{RATIO};
    $line_array->[$ratio_idx] = ($line_array->[$ratio_idx] eq "NA") ? -1.0 : $line_array->[$ratio_idx];

    my @data = @{$line_array}[
        $plink_file_indices->{Z0},
        $plink_file_indices->{Z1},
        $plink_file_indices->{Z2},
        $plink_file_indices->{PHE},
        $plink_file_indices->{DST},
        $plink_file_indices->{PPC},
        $plink_file_indices->{RATIO}
    ];
    return pack( "f f f s f f f", @data);
}
...
sub load_data {
    ...
    while ( my $line = <IN> ) {
        ...
        ## skip empty lines
        if ($line eq "") { 
            my $err_msg = "ERROR: Detected an empty line in the genome file: $file. This error indicates that the file is likely malformed. Please check this file before rerunning COMPADRE";
            $LOG->warn($err_msg); 
            die $err_msg;
        }    
        ...
        my @temp = split( /\s+/, $line );
        # We need to make sure the file has the expected number or columns or otherwise the program will error.
        if (@temp < 14) {
            my $err_msg = "encountered line in file, $file, has insufficient columns.. Expected 14 columns, found " . scalar(@temp) . " columns. This error likely indicates a malformed file. Please check the file and fix any formatting issues before rerunning COMPADRE.\n";
            $LOG->warn($err_msg);
            die $err_msg;
        }
        ...
        my $mapped_id1 = generate_id_mapping($IID1, $FID1, $state);
        my $mapped_id2 = generate_id_mapping($IID2, $FID2, $state);
        # Lets sort the names so that we dont have to store both copies of the key
        my ($id1, $id2) = sort_key( $mapped_id1, $mapped_id2 );
        ...
        my $packed_data = pack_line($config->{genome_file_columns}, \@temp);
...
sub colapse_networks {
    ...
    foreach my $id_pair ( keys %$id_id_scores ) {
        ...
        my $orig_iid1 = (defined $state->{iid_map} && exists $state->{iid_map}->{$id1}) ? $state->{iid_map}->{$id1} : $id1;
        my $orig_iid2 = (defined $state->{iid_map} && exists $state->{iid_map}->{$id2}) ? $state->{iid_map}->{$id2} : $id2;

        my $id1_network = $state->{id_network}{$orig_iid1};
        my $id2_network = $state->{id_network}{$orig_iid2};
        ...
        if ( !$config->{do_PR} ) {
            ...
            #     #print "key: $_\n";
            # }
            if ( !exists $$relationships_ref{$orig_iid1}{$orig_iid2} ) {
                my $temp = $orig_iid1;
                $orig_iid1 = $orig_iid2;
                $orig_iid2 = $temp;
            }

            #print "id1: $orig_iid1; id2: $orig_iid2\n";
            my @vector = @{ $$relationships_ref{$orig_iid1}{$orig_iid2} };
            my @possibilities =
            ...
            if ( @possibilities < 1 ) {
                $LOG->warn("ERROR: No relationship possibilities returned from predict_relationships_2D for pair $orig_iid1, $orig_iid2. This likely indicates a problem with the input .genome file data. Please investigate the issue and fix the input data before running COMPADRE.\n");
            }
...
sub write_out_networks {
    ...
    print "NETWORKS_OUT: $config->{output_dir}/$config->{relatedness_file_name}\_networks\n";
    print "UNREL_OUT: $config->{output_dir}/$config->{relatedness_file_name}\_unrelated_samples.txt\n";
    ...
    foreach my $network ( sort { $a <=> $b } keys %{ $state->{networks} } ) {
        ...
        # use Data::Dumper;
        # print "NETWORK $network: " . Dumper(\@temp) . "\n";
        if ( @temp < 2 ) {
            my $name = @temp[0];
            $name =~ s/\*\*/\t/;
        ...
        for my $i ( 0 .. @temp - 1 ) {
            for ( my $j = 0 ; $j < $i ; $j++ ) {
                ...
                my $m_a = (defined $state->{ids_loaded} && exists $state->{ids_loaded}->{$iid_a}) ? $state->{ids_loaded}->{$iid_a} : $iid_a;
                my $m_b = (defined $state->{ids_loaded} && exists $state->{ids_loaded}->{$iid_b}) ? $state->{ids_loaded}->{$iid_b} : $iid_b;

                my ($orig_iid1, $orig_iid2);
                if (looks_like_number($m_a) && looks_like_number($m_b)) {
                    ($orig_iid1, $orig_iid2) = ($m_a < $m_b) ? ($iid_a, $iid_b) : ($iid_b, $iid_a);
                } else {
                    ($orig_iid1, $orig_iid2) = sort_key($iid_a, $iid_b);
                }

                my $m_id1 = (defined $state->{ids_loaded} && exists $state->{ids_loaded}->{$orig_iid1}) ? $state->{ids_loaded}->{$orig_iid1} : $orig_iid1;
                my $m_id2 = (defined $state->{ids_loaded} && exists $state->{ids_loaded}->{$orig_iid2}) ? $state->{ids_loaded}->{$orig_iid2} : $orig_iid2;

                my $orig_fid1 = (defined $state->{fid_map} && exists $state->{fid_map}->{$m_id1}) ? $state->{fid_map}->{$m_id1} : "";
                my $orig_fid2 = (defined $state->{fid_map} && exists $state->{fid_map}->{$m_id2}) ? $state->{fid_map}->{$m_id2} : "";

                my ($sorted_m_id1, $sorted_m_id2) = sort_key( $m_a, $m_b );
                my $key = "$sorted_m_id1;$sorted_m_id2";
                my $info = $state->{id_id_all_info}{$key};
                my $pi_hat = $state->{id_id_scores}{$key} // 0;
                
                my $output_line;
                # If pair not in original file (synthetic/missing pair), generate on-the-fly
                if (!defined $info) {
                    $output_line = sprintf(
                        "%s %s  %s %s %s %5d  %s  %s  %s  %s  %2d  %s  %s%8s\n",
                        $orig_fid1, $orig_iid1, $orig_fid2, $orig_iid2, "OT", 0,
                        "1.0000", "0.0000", "0.0000", sprintf("%.4f", $pi_hat),
                        -1, "0.000000", "1.0000", "NA"
                    );
                }
                else {
                    # We need to now unpack the string
                    my ($z0, $z1, $z2, $phe, $dst, $ppc, $ratio) = unpack("f f f s f f f", $info);
                    my $ratio_str = ($ratio == -1.0) ? "NA" : sprintf("%.4f", $ratio);
                    $output_line = sprintf(
                        "%s %s  %s %s %s %5d  %s  %s  %s  %s  %2d  %s  %s%8s\n",
                        $orig_fid1, $orig_iid1, $orig_fid2, $orig_iid2, "OT", 0,
                        sprintf("%.4f", $z0),
                ...
                if ( $output_line ne "" ) {
                    print GENOMES_OUT $output_line;
                }
                my $score = $state->{id_id_scores}->{$key};
                if ( $score > $config->{threshold} ) {
                    my $output_line_chomped = $output_line;
                    chomp($output_line_chomped);
                    print NETWORKS_OUT "$network_ctr\t$output_line_chomped\n";
                }
            }
        }
        $network_ctr++;
...
sub get_connectedness {
    ...
    for ( my $i = 0 ; $i < $num_ids ; $i++ ) {
        for ( my $j = $i + 1 ; $j < $num_ids ; $j++ ) {
            my $key        = @ids[$i];
            my $key2       = @ids[$j];
            my $m_id1      = (defined $state->{ids_loaded} && exists $state->{ids_loaded}->{$key}) ? $state->{ids_loaded}->{$key} : $key;
            my $m_id2      = (defined $state->{ids_loaded} && exists $state->{ids_loaded}->{$key2}) ? $state->{ids_loaded}->{$key2} : $key2;
            my ($id1, $id2) = sort_key( $m_id1, $m_id2 );
            my $sorted_key   = "$id1;$id2";
            $max_connections++;
...
sub get_actual_neighbors {
    ...
    for my $n_v ( keys $hash_ref->%*) {
        ...
        # Get the PI_HAT value for the pair using mapped numeric IDs
        my $m_v = (defined $state->{ids_loaded} && exists $state->{ids_loaded}->{$v}) ? $state->{ids_loaded}->{$v} : $v;
        my $m_n_v = (defined $state->{ids_loaded} && exists $state->{ids_loaded}->{$n_v}) ? $state->{ids_loaded}->{$n_v} : $n_v;
        my ($id1, $id2) = sort_key( $m_v, $m_n_v );
        my $key           = "$id1;$id2";
        my $score = $state->{id_id_scores}->{$key};
...
sub write_out_dot_file {
    ...
    for my $i ( 0 .. @temp - 1 ) {
        for ( my $j = 0 ; $j < $i ; $j++ ) {
            ...
            # Pull the info using the sorted mapped id key
            my ($sorted_m_id1, $sorted_m_id2) = sort_key( $m_a, $m_b );
            my $key   = "$sorted_m_id1;$sorted_m_id2";
            my $info  = $state->{id_id_all_info}{$key};
            ...
            if (!defined $info) {
                my $pi_hat = $state->{id_id_scores}{$key} // 0;
                # Synthetic line for unrelated pair: FID1 IID1 FID2 IID2 RT EZ Z0 Z1 Z2 PI_HAT PHE DST PPC RATIO
                $info = "$orig_fid1\t$orig_iid1\t$orig_fid2\t$orig_iid2\tUN\t0\t1\t0\t0\t$pi_hat\t-1\t0\t1\tNA";
            }
            ...
            if ( $score > $config->{threshold} ) {
                my $color = "black";
                print GRAPH_OUT "\t\"$orig_iid1\" -- \"$orig_iid2\" [color=$color];\n";
            }
            elsif ( $network eq "all" ) {
                print GRAPH_OUT "\tnode \"$orig_iid1\";\n";
                print GRAPH_OUT "\tnode \"$orig_iid2\";\n";
            }
        }
    }
    print GRAPH_OUT "}";
