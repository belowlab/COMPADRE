...
sub run_prePRIMUS_main
{
	...
	else ## Use HapMap3 as a reference
	{
		...
		## Past this point will be fed the ryan pop-classifier
...
sub pick_reference_populations
{
	...
	{
		my $file = "$lib_dir/hapmap3/unrelateds_by_subpop/formatted_set_$pop";
	...
	## For each sample, compare its PCV1 and PCV2 to each pop to make a vector
	open(IN,"$data_stem.eigenvec") or die "can't open $data_stem.eigenvec: $!\n";
	open(OUT,">$data_stem\_samples_ancestries.txt") or die "can't open $data_stem\_samples_ancestries.txt: $!\n";
	print OUT "FID\tIID\tpopulation\tLikelihood_vector\n";
	while(my $line = <IN>)
	{
		$line =~ s/^\s+//;
		my ($FID,$IID,$pc1,$pc2, @rest) = split(/\s+/,$line);
	...
	if(@ref_pops < 1)
	{
		die "Your samples did not fall near any samples in the PCA plot. Visual inspection required of PCA plot. Select reference populations and rerun PRIMUS with --ref_pops [POP] option (e.g. --ref_pops CEU,TSI,YRI\n";
	}
	return @ref_pops;
}
...
sub multiple_ref_pop_merge
{
	...
	my $ref_samples_to_keep_stem = "$new_stem\_reference_samples";
    #my $files = "";
	...
	keep_samples($HM3_STEM,"$new_stem\_reference_samples","$ref_samples_to_keep_stem.txt");
	$intermediate_files{"$new_stem\_reference_samples.bed"} = 10;
	$intermediate_files{"$new_stem\_reference_samples.bim"} = 10;
	$intermediate_files{"$new_stem\_reference_samples.fam"} = 10;
