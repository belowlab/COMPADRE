...
# the clique
{
    ...
    my $num_visited = 0;
    use Data::Dumper;
    print "P ref: " . Dumper(\%P);
    print "state id_id_scores: " . Dumper($state->{id_id_scores});
    print "maximal_cliques before: " . Dumper(\@maximal_cliques);
    
    # Call the actual BronKerbosh from IMUS
    PRIMUS::IMUS::BronKerbosh($config, $state, \@maximal_cliques, \%R, \%P, \%X, \$num_visited);
    print "maximal_cliques after: " . Dumper(\@maximal_cliques);
...
# Test 44: Empty unrelated neighbors - Node with no neighbors <= threshold
{
    ...
    PRIMUS::IMUS::get_inverse_neighbors($config, $state, 'ID01', $network_ref, $neighbors_ref);
    use Data::Dumper;
    print "Neighbors ref: " . Dumper($neighbors_ref);
